...
test('stone knife replaces generic tool for corpse processing',()=>{
 const {s,m,a}=local();s.bag={stoneknife:1};delete s.provisions;a.x=5;a.y=6;killAnimal(m,a,s.time);hunt(s,'inspectAnimal',a.id);hunt(s,'butcherAnimal',a.id);hunt(s,'skinAnimal',a.id);hunt(s,'processHide');assert.equal(s.bag.rawmeat,1);assert.equal(s.bag.cloth,1);assert.equal(s.bag.stoneknife,1);E.validateSave(s);
});
