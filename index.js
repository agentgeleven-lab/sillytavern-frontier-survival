...
export function initialize(){
  ...
  async function act(type,p){
    await store.run(async(s,signal,transaction)=>{
      ...
      if(['campBatchPlace','campSelectBed','campSetMode','campAutoPlace','campEnter','campExit','campMove','campPlace','campRelocate','campRemove','campDoor','campDeposit','campWithdraw','campRename'].includes(type)){campAction(s,type,p);return;}
  ...
  globalThis.FrontierSurvival={open:ui.open,version:'0.17.0',getKnownContext:()=>E.knownContext(store.state)};
