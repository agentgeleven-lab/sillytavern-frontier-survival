...
export function initialize(){
  ...
  async function act(type,p){
    await store.run(async(s,signal,transaction)=>{
      ...
      else if(['approachAnimal','inspectAnimal','scareAnimal','attackAnimal','butcherAnimal','skinAnimal','clearAnimal','cookMeat','processHide'].includes(type))huntingAction(s,type,p.actor);
  ...
  globalThis.FrontierSurvival={open:ui.open,version:'0.10.0',getKnownContext:()=>E.knownContext(store.state)};
