...
export const PROMPTS = {
  world: base+'生成一个区域地图。严格使用 constraints.dimension 指定的宽高（9、15或21），不是生成整个世界。返回 {"name":"区域名","description":"概述","terrain":[等长字符串行],"sites":[{"x":4,"y":4,"name":"建筑名","kind":"用途","size":"small或normal或large","description":"外观"}]}。地形字符只允许 .草地 f森林 r道路 w水域 h丘陵。constraints.boundaries 是每个边界坐标必须使用的地形，逐一遵守；constraints.edges 提供相邻边缘的道路河流连续性，参考 neighbors 的地形与名字自然过渡。中心坐标为floor(dimension/2)，必须是陆地，全部建筑及四个 gates 必须能从中心沿四方向陆地抵达。布置3–12个有用途差异的建筑，大小按用途选择，不需要塞满区域；至少一个位于中心3×3范围。不得返回角色数值、战利品或其他区域地图。',
