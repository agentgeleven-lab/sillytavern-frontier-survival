...
export const PROMPTS = {
  ...
  loot: base+'生成本次搜索或采集结果，返回 {"description":"玩家实际看到与获得的结果","items":[{"id":"water","qty":1}]}。id只允许 wood木材 cloth布料 scrap金属零件 water饮用水 food食品 berries浆果 vegetables蔬菜 grain谷物 canned罐头 fish鱼肉 rawmeat生肉 herb草药 simpledrug简易药物 drugs药物 bandage绷带 medkit医药箱 vegseed蔬菜种子 grainseed谷物种子 herbseed草药种子 tool工具 fuel燃料 torch火把 flashlight手电 battery电池 stone石料 fiber植物纤维 components通用组件 parts机械零件 electronics电子元件 book书籍 warmcoat保暖外套。每种最多出现一次、数量1–5，总种类0–4。物品必须符合地点与容器，纯自然地点不能出现遗留包装、药箱或人造工具；植物纤维使用fiber，手电与电池主要出现在人工建筑内，未经处理的天然水不能算饮用水。人工建筑可少量出现 components（通用组件）、parts（机械零件）、electronics（电子元件）、book（书籍）、warmcoat（保暖外套），高级材料较稀有；这些也是允许的物品 id。药物、绷带和医药箱只能来自人工地点搜索，不能从自然环境生成；medkit 每箱完整五次。种子可见于农场和园艺容器。资源稀缺，允许空结果。不要添加敌人、伤害、位移、额外时间或未结算行动。数量描述必须与items完全一致。',
