...
export const TERRAINS = { '.': ['草地', '#54664b'], f: ['林地', '#344d41'], r: ['道路', '#77746a'], w: ['水域', '#365d68'], h: ['丘陵', '#736854'] };
export const ITEMS = {
...
export function validateWorld(raw) {
  assert(raw && Array.isArray(raw.terrain) && raw.terrain.length === 9, '区域地图需要 9 行地形');
  const cells = {};
  raw.terrain.forEach((row, y) => {
    assert(typeof row === 'string' && row.length === 9 && [...row].every(c => Object.hasOwn(TERRAINS, c)), '区域地形需要 9×9 个合法格子');
    [...row].forEach((terrain, x) => { cells[key(x, y)] = { x, y, terrain, name: TERRAINS[terrain][0], known: false, visited: false, surveyed: false, depleted: false, site: null, camp: null }; });
  });
  assert(cells['4,4'].terrain !== 'w', '起点不能位于水中');
  assert(Array.isArray(raw.sites) && raw.sites.length <= 15, '建筑数量不合法');
  for (const site of raw.sites) {
    const c = cells[key(int(site.x, 0, 8), int(site.y, 0, 8))];
    assert(c.terrain !== 'w' && !c.site, '建筑重叠或位于水中');
    c.site = { id: `site-${c.x}-${c.y}`, name: text(site.name, 70), kind: text(site.kind, 50), description: text(site.description, 400), origin: 'world' };
    assert(c.site.name, '建筑需要名称'); c.name = c.site.name;
  }
  const connected = flood({ x: 4, y: 4 }, (x, y) => !!cells[key(x, y)] && cells[key(x, y)].terrain !== 'w');
  assert(Object.values(cells).filter(c=>c.site).every(c=>connected.has(key(c.x,c.y))), '建筑必须能够从起点抵达');
  return { name: text(raw.name, 80), description: text(raw.description, 1000), size: 9, cells };
}
export function newGame(raw, { background = '', seed = 'frontier', mode = 'api', theme = 'waste' } = {}) {
  const s = { version: VERSION, id: globalThis.crypto.randomUUID(), revision: 0, seed, mode, theme, background: text(background, 30000),
    world: validateWorld(raw), player: { x: 4, y: 4, local: null }, time: 480, weather: '晴',
...
export function validateLocal(raw) {
  assert(raw&&Array.isArray(raw.grid)&&raw.grid.length>=8&&raw.grid.length<=20,'局部地图高度必须为 8–20');
  const grid=raw.grid.map(row=>text(row,24)),w=grid[0].length,h=grid.length;
  assert(w>=10&&w<=24&&grid.every(row=>row.length===w&&/^[.#+=E]+$/.test(row)),'局部地图行宽或地形符号不合法');
  const exits=[],doors={};
  grid.forEach((row,y)=>[...row].forEach((tile,x)=>{if(tile==='E')exits.push({x,y});if(tile==='+')doors[key(x,y)]=false;if(x===0||y===0||x===w-1||y===h-1)assert('#=E'.includes(tile),'局部地图边缘必须封闭，出口使用 E');}));
  assert(exits.length===1,'第一版局部地图需要且只允许一个区域出入口');
  const start=exits[0];assert(start.x===0||start.y===0||start.x===w-1||start.y===h-1,'区域出口必须位于地图边缘');
  assert(Array.isArray(raw.containers)&&raw.containers.length>=1&&raw.containers.length<=12,'需要 1–12 个可搜索容器');
  ...
  return {w,h,grid,doors,containers,exit:start,seen:{},description:text(raw.description,1200)};
}
...
export function visible(map, from, x, y) {
  ...
  for(let i=1;i<steps;i++){const a=Math.round(from.x+(x-from.x)*i/steps),b=Math.round(from.y+(y-from.y)*i/steps),t=tileAt(map,a,b);if(t==='#'||(t==='+'&&!map.doors[key(a,b)]))return false;}
  return true;
}
...
export function enter(s, raw) {
  ...
  if(!s.locals[c.site.id]){assert(raw,'需要先生成建筑布局');s.locals[c.site.id]=validateLocal(raw);}
...
export function knownContext(s){if(!s)return '';const c=cell(s),map=localMap(s);return JSON.stringify({world:s.world.name,time:formatTime(s.time),weather:s.weather,position:{x:s.player.x,y:s.player.y,location:c.name,roomPosition:s.player.local?{x:s.player.local.x,y:s.player.local.y}:null},status:Object.fromEntries(Object.entries(s.stats).map(([k,v])=>[k,Math.round(v*10)/10])),inventory:s.bag,camp:c.camp,nearby:Object.values(s.world.cells).filter(c=>c.known&&Math.abs(c.x-s.player.x)<=2&&Math.abs(c.y-s.player.y)<=2).map(c=>({x:c.x,y:c.y,name:c.name,visited:c.visited})),visibleObjects:map?Object.values(map.containers).filter(c=>map.seen[key(c.x,c.y)]).map(c=>({name:c.name,x:c.x,y:c.y,searched:c.searched,...(c.searched?{items:c.items}:{})})):[],clues:s.clues.filter(c=>!c.revoked).slice(-10).map(c=>({kind:c.kind,title:c.title,detail:c.detail,x:c.x,y:c.y,source:c.sourceName})),recent:s.log.slice(-5).map(e=>e.text)});}
...
export function validateSave(s){
  ...
  assert(s&&s.version===VERSION&&typeof s.id==='string'&&s.id.length<100&&s.world?.size===9,'存档格式或版本不支持');
  ...
  assert(Object.keys(s.world.cells??{}).length===81&&Array.isArray(s.log)&&s.log.length<=120&&Array.isArray(s.clues)&&s.clues.length<=2000&&s.sync&&s.locals,'存档结构损坏');
  int(s.player.x,0,8);int(s.player.y,0,8);int(s.time,0,1e9);int(s.revision,0,1e9);
  ...
  for(let y=0;y<9;y++)for(let x=0;x<9;x++){const c=cell(s,x,y);assert(c&&c.x===x&&c.y===y&&Object.hasOwn(TERRAINS,c.terrain),'地图格子无效');text(c.name,80);if(c.site){text(c.site.id,200);text(c.site.name,80);text(c.site.description,1000);assert(!siteIds.has(c.site.id),'建筑标识重复');siteIds.add(c.site.id);}if(c.camp){int(c.camp.level,1,2);inventory(c.camp.storage);}}
  assert(Object.keys(s.locals).length<=81,'局部地图过多');
  for(const[id,map]of Object.entries(s.locals)){
    assert(siteIds.has(id)&&map.seen&&map.doors&&map.containers,'局部地图缺少对应建筑或状态');
    const checked=validateLocal({description:map.description,grid:map.grid,containers:Object.values(map.containers)});
  ...
  for(const e of s.log){text(e.text,1500);int(e.time,0,1e9);}for(const c of s.clues){text(c.id,200);text(c.title,80);text(c.detail,800);assert(['rumor','discovery'].includes(c.kind),'线索类型无效');assert((c.x===null&&c.y===null)||(Number.isInteger(c.x)&&Number.isInteger(c.y)&&c.x>=0&&c.x<=8&&c.y>=0&&c.y<=8),'线索坐标无效');}
  return s;
}
