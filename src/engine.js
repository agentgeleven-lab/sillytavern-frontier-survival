...
export function tick(s, minutes, effort = 1, updateSight = true) {
  ...
  applyDevLocks(s);s.ended=s.stats.health<=0;if(updateSight){refreshSight(s);revealCamp(s);}
}
...
export function useItem(s,id){assert(!s.ended&&s.stats.health>0,'角色已无法行动');assert((s.bag[id]??0)>0&&['water','food','medicine','cookedmeat','smokedmeat'].includes(id),'没有可用的消耗品');assert(id!=='medicine'||!Object.values(injuryState(s)).some(v=>v>0),'存在伤势，请使用伤势面板的医疗处理');if(Object.hasOwn(FOOD_LIFE,id))consumeFood(s,s.bag,id);else s.bag[id]--;const stat={smokedmeat:'food',water:'water',food:'food',cookedmeat:'food',medicine:'health'}[id];s.stats[stat]=Math.min(100,s.stats[stat]+(id==='medicine'?25:30));log(s,`使用${ITEMS[id].name}。`);}
...
export function rest(s){if(s.player.camp&&cell(s).camp?.layout){const cover=campRestCover(s),level=cover.kind==='room'?cell(s).camp.level:0,rate=level?15+level*12:cover.covered?21:15,health=level?level*4:cover.covered?2:0;for(let i=0;i<60;i++){const rain=!cover.covered&&s.weather==='小雨';tick(s,1,0,false);if(s.ended)break;s.stats.stamina=Math.min(100,s.stats.stamina+rate/60*(rain?.5:1));if(s.stats.food>20&&s.stats.water>20)s.stats.health=Math.min(100,s.stats.health+health/60);healWounds(s,1,hasFacility(s,'bed'));applyDevLocks(s);}log(s,`休息一小时：${cover.label}。`);return;}const level=!s.player.local&&(!cell(s).camp?.layout||s.player.camp)?cell(s).camp?.level??0:0;tick(s,60,0);if(s.ended){log(s,'休息中健康耗尽，无法继续恢复。');return;}healWounds(s,60,hasFacility(s,'bed'));s.stats.stamina=Math.min(100,s.stats.stamina+15+level*12);if(level&&s.stats.food>20&&s.stats.water>20)s.stats.health=Math.min(100,s.stats.health+level*4);log(s,`休息一小时${level?'，庇护所改善了恢复效果':''}。`);}
...
export function validateSave(s){
  ...
  assert(s.stats.spirit===undefined||(Number.isFinite(s.stats.spirit)&&s.stats.spirit>=0&&s.stats.spirit<=100),'精神值无效');validateDeveloper(s);validateCamps(s,inventory);validateFood(s);validateFacilities(s);validateInjuries(s);validateEquipment(s);validateLighting(s);validateSleep(s);validateWildlife(s);return s;
}
