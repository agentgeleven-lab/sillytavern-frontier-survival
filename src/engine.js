...
export function tick(s, minutes, effort = 1, updateSight = true, thermalMode = 'normal') {
  ...
  advanceTemperature(s,previousTime,effort,thermalMode);advanceStomach(s,previousTime);settleRain(s,previousTime);settleGardens(s);advanceBenefits(s,previousTime);applyDevLocks(s);s.ended=s.stats.health<=0;if(updateSight){refreshSight(s);revealCamp(s);}
}
