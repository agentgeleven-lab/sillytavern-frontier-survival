...
import {ensureEncounters,npcMinute,validateEncounters,visibleNpcs} from './encounter-data.js';
import {occupied} from './combat-data.js';
import {DURABLE_ITEMS,worn,validateDurables} from './durable-data.js';
import {validateCombat} from './combat-data.js';
import {SEEDS,settleGardens,validateGardens} from './garden-data.js';
import {MEDICAL_ITEMS,validateMedical,medicalRecovery,coldLevel,medicalConditions} from './medical-data.js';
import {FOODS,EDIBLE_FOODS} from './food-data.js';
...
export const ITEMS = {
  ...PACKED,...TRAP_ITEMS,...DURABLE_ITEMS,...MATERIALS,...MEDICAL_ITEMS,...SEEDS,
  ...Object.fromEntries(Object.entries(FOODS).map(([id,r])=>[id,{name:r.name,weight:r.weight}])),
  fishingrod:{name:'钓竿',weight:.8},
  ...
  water: { name: '饮用水', weight: .5 }, food: { name: '食品', weight: .3 }, medicine: { name: '旧医疗用品', weight: .1 },
...
export function tick(s, minutes, effort = 1, updateSight = true, thermalMode = 'normal') {
  ...
  ensureEncounters(s,localMap(s));if(timeFrozen(s)){ensureWildlife(s,localMap(s));return;}prepareFood(s);const previousTime=s.time;s.time += minutes;advanceInjuries(s,minutes);advanceSpirit(s,previousTime,updateSight);advanceWildlife(s,previousTime,updateSight);if(localMap(s)&&!localMap(s).kind)for(let t=previousTime+1;t<=s.time;t++)npcMinute(s,localMap(s),t);const exhausted=burnPortable(s,minutes);if(exhausted)log(s,exhausted);
  ...
  advanceTemperature(s,previousTime,effort,thermalMode);advanceStomach(s,previousTime);settleRain(s,previousTime);settleGardens(s);advanceEvents(s,previousTime);advanceBenefits(s,previousTime);applyDevLocks(s);s.ended=s.stats.health<=0;if(updateSight){refreshSight(s);revealCamp(s);}
}
...
export function validateLocal(raw, size='normal', saved=false) {
...
export function take(s,x,y,id,qty=1){const c=localMap(s)?.containers[key(x,y)];assert(c?.searched&&!c.removed&&!c.resourceId&&adjacent(s,x,y),'需要在已搜索的容器旁');transfer(c.items,s.bag,id,qty,20,s);log(s,`拿取${ITEMS[id].name} ×${qty}。`);}
export function transfer(from,to,id,qty,limit=20,s=null){if(s&&from===s.bag&&Object.values(worn(s)).includes(id))assert((from[id]??0)-qty>=1,'请先脱下穿戴的最后一件装备');assert(Object.hasOwn(ITEMS,id)&&Number.isInteger(qty)&&qty>0&&(from[id]??0)>=qty,'物品数量不足');assert(weight(to)+ITEMS[id].weight*qty<=limit,'背包负重超过 20 kg');if(s)transferFood(s,from,to,id,qty);else {from[id]-=qty;to[id]=(to[id]??0)+qty;}}
export function useItem(s,id){assert(!s.ended&&s.stats.health>0,'角色已无法行动');assert((s.bag[id]??0)>0&&['water','medicine',...EDIBLE_FOODS].includes(id),'没有可用的消耗品');assert(id!=='medicine'||!Object.values(injuryState(s)).some(v=>v>0),'存在伤势，请使用伤势面板的医疗处理');const foodBefore=s.stats.food;if(Object.hasOwn(FOOD_LIFE,id))consumeFood(s,s.bag,id);else s.bag[id]--;const meal=FOODS[id];if(meal){s.stats.food=Math.min(100,s.stats.food+(meal.food??0));s.stats.water=Math.min(100,s.stats.water+(meal.water??0));s.stats.spirit=Math.min(100,(s.stats.spirit??100)+(meal.spirit??0));}else{const stat=id==='medicine'?'health':'water';s.stats[stat]=Math.min(100,s.stats[stat]+(id==='medicine'?25:30));}if(mealBenefit(s,id,foodBefore))log(s,'获得饱腹满足：持续 4 小时。');log(s,`使用${ITEMS[id].name}。`);}
...
export function validateSave(s){
  ...
  for(const[id,map]of Object.entries(s.locals)){
    ...
    const checked=validateLocal({description:map.description,grid:map.grid,containers:Object.values(map.containers).filter(c=>!c.removed)},map.kind?map.size:Object.values(s.world.cells).find(c=>c.site?.id===id).site.size??'normal',true);
    ...
    assert(Object.keys(map.containers).length<=60,'容器记录过多');for(const[k,c]of Object.entries(map.containers)){text(c.name,60);text(c.kind,50);assert(k===key(c.x,c.y)&&Number.isInteger(c.x)&&Number.isInteger(c.y)&&c.x>0&&c.y>0&&c.x<map.w-1&&c.y<map.h-1&&map.grid[c.y][c.x]==='.'&&typeof c.searched==='boolean','容器状态无效');inventory(c.items);}
...
export function demoLoot(kind){if(kind==='survey')return {items:[{id:'wood',qty:3},{id:'cloth',qty:1}],description:'收集到可用的干木料和遗弃的帆布。'};if(/工具|衣/.test(kind??'')&&!kind?.includes('医疗'))return {items:[{id:'cap',qty:1},{id:'trousers',qty:1},{id:'scrap',qty:2}],description:'找到一顶皮帽、厚布裤和一些金属零件。'};if(kind?.includes('医疗'))return {items:[{id:'drugs',qty:2},{id:'bandage',qty:2},{id:'medkit',qty:1}],description:'找到药物、绷带和一只完整医药箱。'};if(/食品|货架|粮|厨房/.test(kind??''))return {items:[{id:'canned',qty:1},{id:'grain',qty:2},{id:'vegetables',qty:2},{id:'vegseed',qty:1}],description:'找到密封罐头、干燥谷物、可食蔬菜和蔬菜种子。'};return {items:[{id:'water',qty:2},{id:'food',qty:1},{id:'cloth',qty:2},{id:'book',qty:1},{id:'scrap',qty:1}],description:'清理灰尘后，找到饮用水、食品、布料、一本书与金属零件。'};}
