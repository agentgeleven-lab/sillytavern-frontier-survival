export const DEV_STATS={health:'健康',food:'饱食',water:'水分',stamina:'体力',spirit:'精神',trauma:'创伤',bleeding:'出血',infection:'感染'};
export const devEnabled=s=>s?.developer?.enabled===true;
export const timeFrozen=s=>devEnabled(s)&&s.developer.timeLocked;
export const devValue=(s,id)=>['trauma','bleeding','infection'].includes(id)?s.injuries?.[id]??0:s.stats[id]??100;
export function setDevValue(s,id,value){if(['trauma','bleeding','infection'].includes(id)){s.injuries??={trauma:0,bleeding:0,infection:0,exposure:0};s.injuries[id]=value;if(id==='bleeding'&&!value)s.injuries.exposure=0;}else s.stats[id]=value;}
