...
export function actorStep(s,m,a,target,flee=false){const valid=(x,y)=>!blocked(m,x,y)&&(a!==s.player.local||m.seen[`${x},${y}`])&&(!a.species||x>0&&y>0&&x<m.w-1&&y<m.h-1&&m.grid[y][x]==='.'&&!m.portals?.some(p=>p.x===x&&p.y===y));const dirs=[[1,0],[-1,0],[0,1],[0,-1]];if(flee)return dirs.map(([dx,dy])=>({x:a.x+dx,y:a.y+dy})).filter(p=>valid(p.x,p.y)&&!occupied(s,m,p.x,p.y,a)&&distance(p,target)>distance(a,target)).sort((a,b)=>distance(b,target)-distance(a,target))[0];const q=[{x:a.x,y:a.y,first:null}],seen=new Set([`${a.x},${a.y}`]);for(let i=0;i<q.length&&i<2000;i++){const p=q[i];if(localTouch(m,p,target))return p.first;for(const[dx,dy]of dirs){const x=p.x+dx,y=p.y+dy,k=`${x},${y}`;if(!seen.has(k)&&valid(x,y)&&!occupied(s,m,x,y,a)){seen.add(k);q.push({x,y,first:p.first??{x,y}});}}}return null;}
