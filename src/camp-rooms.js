import {campSize,campAt,tiles,canUseCampObject} from './camp-data.js';
const k=(x,y)=>`${x},${y}`,dirs=[[1,0],[-1,0],[0,1],[0,-1]],cache=new WeakMap();
export function campRooms(l){
 const n=campSize(l),signature=n+':'+l.objects.filter(o=>['wall','door','roof'].includes(o.type)).map(o=>`${o.type}:${o.x},${o.y}:${o.open}`).join('|'),old=cache.get(l);if(old?.signature===signature)return old.value;
 ...
 for(let y=1;y<n-1;y++)for(let x=1;x<n-1;x++){const key=k(x,y);if(seen.has(key)||blocked.has(key))continue;const points=[{x,y}],region={id:regions.length+1,points,closed:true,roofed:0,holes:[],openDoors:[]};seen.add(key);
  for(let i=0;i<points.length;i++){const p=points[i],pk=k(p.x,p.y);byTile[pk]=region;if(roofs.has(pk))region.roofed++;else region.holes.push(p);if(p.x===1||p.y===1||p.x===n-2||p.y===n-2)region.closed=false;for(const[dx,dy]of dirs){const a=p.x+dx,b=p.y+dy,key=k(a,b);if(a>=1&&b>=1&&a<=n-2&&b<=n-2&&!seen.has(key)&&!blocked.has(key)){seen.add(key);points.push({x:a,y:b});}}}
