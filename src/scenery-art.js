...
export function sceneryTree(g,x,y,size,e,snow=false){const style=sceneryOf(e),p=palettes[style]??palettes.broadleaf;if(drawSprite(g,snow?'snowpine':style==='jungle'?'jungle':['taiga','coldmeadow','alpine','snowfield'].includes(style)?'pine':'broadleaf',x-size*.24,y-size*.45,size*.48,size*.65))return;g.save();g.translate(x,y);g.scale(size,size);oval(g,.04,.15,.18,.07,'#263d3033');g.fillStyle='#79644a';g.fillRect(-.02,-.02,.04,.22);
...
export function scenicTerrain(g,t,x,y,seed,neighbor,e){if(!['f','h','n','.'].includes(t))return false;const style=sceneryOf(e),p=palettes[style]??palettes.meadow,r=i=>randomAt(seed+':art',x*31+i,y*29-i);g.fillStyle=t==='n'?palettes.snowfield[0]:t==='h'?(palettes[style]??palettes.crags)[0]:p[0];g.fillRect(0,0,1,1);
 drawSprite(g,t==='n'?'snow':t==='h'?(style==='alpine'?'snow':'gravel'):t==='f'?'forestEarth':style==='coldmeadow'?'coldgrass':'grass',0,0,1,1,{fill:true,alpha:.48});
 ...
 if(t==='h'){
  if(drawSprite(g,style==='hills'?'hill':style==='alpine'?'snowridge':'ridge',.02,.02,.96,.96))return true;
