import {drawSprite,resourceSprite,containerSprite,buildingSprite,trapSprite,fireSprite} from './assets.js';
import {drawWall} from './wall-art.js';
...
function rock(g,x,y,k=1){if(drawSprite(g,'boulder',x-.27*k,y-.3*k,.54*k,.5*k))return;g.save();g.translate(x,y);g.scale(k,k);path(g,[[-.2,.1],[-.12,-.08],[.04,-.15],[.18,-.01],[.24,.12]],'#7a7e71');path(g,[[-.12,-.08],[.04,-.15],[.02,.08],[-.2,.1]],'#bfc1a3');g.restore();}
function roof(g,large=false,site=null){if(drawSprite(g,buildingSprite(site??{size:large?'large':'small'}),.03,.04,.94,.92))return;g.lineWidth=.018;g.lineCap='butt';ellipse(g,.55,.63,.36,.23,'#3c433b33');g.fillStyle='#d2c3a0';g.fillRect(.21,.34,.59,.38);path(g,[[.12,.38],[.5,.14],[.88,.38],[.5,.61]],large?'#879a91':'#ad7758','#5f6957');path(g,[[.12,.38],[.5,.14],[.5,.61]],large?'#697f79':'#916447');g.strokeStyle='#f0d8a366';for(let i=0;i<3;i++){g.beginPath();g.moveTo(.24+i*.09,.32-i*.06);g.lineTo(.5+i*.09,.48-i*.06);g.stroke();}g.fillStyle='#4a5747';g.fillRect(.59,.6,.11,.14);g.fillStyle='#807254';g.fillRect(.65,.17,.07,.16);}
function pointArt(g,kind){if(drawSprite(g,({cave:'cave',overlook:'ridge',grove:'log',driftwood:'log',berries:'berries',reeds:'reeds'}[kind]),.06,.06,.88,.88))return;g.lineWidth=.02;
...
function camp(g){if(drawSprite(g,'tent',.08,.1,.84,.84))return;g.lineWidth=.02;path(g,[[.27,.78],[.54,.3],[.91,.78]],'#ddc38b','#817451');path(g,[[.54,.3],[.66,.78],[.91,.78]],'#b78d61');path(g,[[.4,.78],[.54,.48],[.61,.78]],'#5d694f');}
function terrain(g,t,x,y,seed,neighbor,e){
  if(scenicTerrain(g,t,x,y,seed,neighbor,e))return;
  g.fillStyle=colors[t]??colors['.'];g.fillRect(0,0,1,1);drawSprite(g,({w:'shallow',s:'sand',m:'marsh',a:'farmland',u:'cobble',r:'dirt'}[t]),0,0,1,1,{fill:true,alpha:.6});g.lineWidth=.012;
  if(t==='w'&&neighbor){for(const[dx,dy]of [[-1,0],[1,0],[0,-1],[0,1]]){const land=neighbor(x+dx,y+dy);if(land&&land!=='w'&&land!=='?'){g.fillStyle=colors[land]??colors.s;g.fillRect(dx===1?.91:0,dy===1?.91:0,dx?.09:1,dy?.09:1);g.strokeStyle='#d9d9b699';g.lineWidth=.02;g.beginPath();if(dx){g.moveTo(dx===1?.9:.1,0);g.lineTo(dx===1?.9:.1,1);}else{g.moveTo(0,dy===1?.9:.1);g.lineTo(1,dy===1?.9:.1);}g.stroke();}}}
  const r=i=>randomAt(seed,x*17+i,y*19-i);
  // Quiet grain stays stable across redraws and under the optional raster atlas.
  ...
  if(t==='r'){
    ...
    g.lineCap='round';for(const [width,color]of [[.46,'#74765f'],[.36,'#c6b992']]){g.strokeStyle=color;g.lineWidth=width;g.beginPath();for(const[,,a,b]of ends){g.moveTo(.5,.5);g.lineTo(a,b);}if(!ends.length){g.moveTo(.4,.5);g.lineTo(.6,.5);}g.stroke();}g.save();g.beginPath();g.arc(.5,.5,.17,0,Math.PI*2);for(const[,,a,b]of ends){if(a===.5)g.rect(.33,Math.min(b,.5),.34,.5);else g.rect(Math.min(a,.5),.33,.5,.34);}g.clip();drawSprite(g,['city','industrial','suburb','town'].includes(e?.landUse)?'asphalt':'dirt',0,0,1,1,{fill:true,alpha:.65});g.restore();
  }
}
...
function indoor(g,map,x,y){const t=map.grid[y][x];g.fillStyle=t==='_'?'#d1d0bb':'#c4b38e';g.fillRect(0,0,1,1);g.lineWidth=.016;if(t==='_')return;
  ...
  drawSprite(g,'wood',0,0,1,1,{fill:true,alpha:.55});
  if(['#','=','+'].includes(t))drawWall(g,(a,b)=>map.grid[b]?.[a],x,y,{open:!!map.doors[key(x,y)]});
  if(t==='E'){drawWall(g,(a,b)=>map.grid[b]?.[a],x,y,{tile:'E',open:true});g.fillStyle='#71947c';g.fillRect(.3,.3,.4,.4);path(g,[[.5,.16],[.23,.48],[.41,.48],[.41,.77],[.59,.77],[.59,.48],[.77,.48]],'#e6e7c3');}
  const c=map.containers[key(x,y)];if(c&&!c.removed&&!drawSprite(g,containerSprite(c),.08,.08,.84,.84)){g.fillStyle='#4d503d44';g.fillRect(.19,.24,.74,.68);g.fillStyle=c.searched?'#968565':'#9f7850';g.fillRect(.12,.12,.72,.7);g.strokeStyle='#dcc095';g.strokeRect(.16,.16,.64,.62);if(/箱/.test(c.kind)){path(g,[[.2,.23],[.76,.71]],null,'#d7b788');path(g,[[.76,.23],[.2,.71]],null,'#d7b788');}else{for(const i of [.35,.58]){g.beginPath();g.moveTo(.16,i);g.lineTo(.79,i);g.stroke();g.fillStyle='#e3cc97';g.fillRect(.4,i+.07,.17,.028);}}}
}
function outdoor(g,s,map,x,y){
  if(map.shore){const t=map.grid[y][x];terrain(g,x>=5?'w':'s',x,y,s.seed,undefined,s.world.environment);if(x>=5&&waterDepth(s,cell(s,map.shore.x,map.shore.y))==='deep'){drawSprite(g,'deep',0,0,1,1,{fill:true,alpha:.65});g.fillStyle='#193f6833';g.fillRect(0,0,1,1);}if(t==='E'){g.fillStyle='#e8d6a4';g.fillRect(.15,.15,.7,.7);}if(x===4&&y===5)pointArt(g,'spring');else if(t==='#'&&x<5)rock(g,.5,.5,1.6);return;}
  ...
  for(const p of map.portals)if(p.x===x&&p.y===y){if(p.kind==='cave')pointArt(g,'cave');else roof(g,c.site?.size==='large',c.site);}
  if(map.fire?.x===x&&map.fire.y===y&&!drawSprite(g,fireSprite(map.fire,fireRemaining(s,map.fire)),.05,.05,.9,.9)){rock(g,.5,.67,1.1);if(canSee(s,x,y)&&map.fire.lit&&fireRemaining(s,map.fire)>0){path(g,[[.3,.64],[.4,.3],[.5,.45],[.6,.15],[.75,.65]],'#d18c47');path(g,[[.4,.65],[.53,.4],[.63,.66]],'#f5d888');}}
  const o=map.containers[key(x,y)];if(o){const n=c.resources.nodes[o.resourceId];if(drawSprite(g,resourceSprite(n),.07,.07,.86,.86))return;g.save();if(!n.remaining)g.globalAlpha=.32;const art=RESOURCES[n.kind].art;if(art==='stone')rock(g,.5,.52,1.7);else pointArt(g,art);g.restore();if(!n.remaining){g.strokeStyle='#6b705c';g.lineWidth=.035;g.beginPath();g.moveTo(.28,.72);g.lineTo(.72,.28);g.stroke();}}
}
...
export function paintMap(canvas,s,{pan,selection,mapLevel}){
  ...
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
    ...
    if(!map&&known){if(activeEvents(s,c).length)ellipse(g,.2,.2,.08,.08,'#d29343');if(c.traps?.length){drawSprite(g,trapSprite(c.traps[0]),.55,.55,.4,.4);g.strokeStyle='#eee1a4';g.lineWidth=.04;g.strokeRect(.65,.65,.22,.22);}}
  ...
  if(map){for(const a of knownWildlife(s).visible){g.save();g.translate(ox+a.x*unit,oy+a.y*unit);g.scale(unit,unit);const def=SPECIES[a.species];if(drawSprite(g,a.species+(a.state==='尸体'?'Dead':''),.06,.04,.88,.86)){g.restore();continue;}ellipse(g,.5,.7,.28,.1,'#253a3433');if(a.state==='尸体'){path(g,[[.24,.3],[.76,.7]],null,'#72594b');path(g,[[.76,.3],[.24,.7]],null,'#72594b');}else{ellipse(g,.48,.54,.23,.14,def.color);ellipse(g,.7,.43,.12,.11,def.color);g.strokeStyle=def.color;g.lineWidth=.045;for(const x of [.32,.57]){g.beginPath();g.moveTo(x,.59);g.lineTo(x,.76);g.stroke();}path(g,[[.65,.35],[.66,.19],[.76,.36]],def.color);if(a.species==='rabbit')path(g,[[.73,.35],[.83,.12],[.81,.4]],def.color);if(a.species==='deer'){path(g,[[.67,.32],[.59,.12],[.56,.2]],null,def.color);path(g,[[.7,.3],[.77,.12],[.8,.19]],null,def.color);}if(a.species==='bat'){path(g,[[.5,.45],[.13,.19],[.19,.59],[.5,.61],[.87,.59],[.9,.19]],def.color);}}g.restore();}}
  if(map)for(const a of visibleNpcs(s)){g.save();g.translate(ox+a.x*unit,oy+a.y*unit);g.scale(unit,unit);const color=a.hp<=0?'#807466':a.hostile?'#b75c4a':'#52799a';if(!drawSprite(g,a.hp<=0?'humanDead':a.kind,.1,.02,.8,.82)){ellipse(g,.5,.3,.13,.13,color);path(g,[[.3,.78],[.35,.49],[.65,.49],[.7,.78]],color);}g.strokeStyle=color;g.lineWidth=.025;g.strokeRect(.1,.03,.8,.82);g.fillStyle='#263d32';g.fillRect(.15,.88,.7,.05);g.fillStyle=color;g.fillRect(.15,.88,.7*a.hp/NPC_KINDS[a.kind].hp,.05);g.restore();}
  if(map&&unit>=20){g.save();g.translate(ox+p.x*unit,oy+p.y*unit);g.scale(unit,unit);g.strokeStyle='#fff0bc';g.lineWidth=.035;g.strokeRect(.08,.04,.84,.88);if(!drawSprite(g,'player',.12,.04,.76,.86))marker(g,.5,.5,.15);g.restore();}else marker(g,ox+(p.x+.5)*unit,oy+(p.y+.5)*unit,Math.max(5,unit*.15));
