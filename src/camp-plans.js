import {FURNITURE,campAt,placementError} from './camp-data.js';
export function strokeTiles(start,end,shape='line'){
 const valid=p=>p&&Number.isInteger(p.x)&&Number.isInteger(p.y)&&p.x>=1&&p.y>=1&&p.x<=13&&p.y<=13;if(!valid(start)||!valid(end))throw Error('请选择营地内部的起点与终点');
...
export function batchPlan(s,type,points){
 if(!['floor','wall','roof'].includes(type)||!Array.isArray(points)||points.length<1||points.length>169)throw Error('批量建设仅支持地板、墙和屋顶，最多 169 格');
