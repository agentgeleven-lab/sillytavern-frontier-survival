import {settleGardens} from './garden-data.js';
...
export function campAction(s,action,p={}){
 ...
 E.assert(!s.ended,'角色已无法行动');settleGardens(s);
 if(action==='campEnter'){E.assert(!s.player.local&&!s.player.camp,'请先返回区域地图');ensureCamp(s);settleRain(s);s.player.camp={...campExit(campAt(s).layout)};revealCamp(s);E.log(s,'进入庇护所营地。');return;}
 ...
 if(action==='campPlace'||action==='campRelocate'){
  const old=action==='campRelocate'?target(s,p,false):null,type=old?.type??p.type;E.assert(!old?.plant,'请先收获或清理作物再搬动');E.assert(Object.hasOwn(FURNITURE,type),'未知家具');E.assert(!old||old.type!=='rainCollector'||!old.rain,'先取空雨水收集器再搬动');const f=FURNITURE[type],o={...(old??{}),id:old?.id??'f'+(l.serial+1),type,x:Number(p.x),y:Number(p.y),rot:Number(p.rot),name:old?.name??f.name};
