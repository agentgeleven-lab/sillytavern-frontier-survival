...
import {FURNITURE,CAMP_EXIT,campAt,nearObject,nearCampFacility,campPath,placementError,revealCamp,campVisible} from './camp-data.js';
import {settleFood} from './provisions.js';
import {equipped,stealth} from './equipment-data.js';
import {lightState} from './lighting-data.js';
import {injuryDelay} from './injury-data.js';
...
export const campStepMinutes=s=>timeFrozen(s)?0:(stealth(s)?2:1)+injuryDelay(s);
function target(s,p){const o=campAt(s).layout.objects.find(o=>o.id===p.id);E.assert(o,'家具不存在');E.assert(nearObject(s.player.camp,o),'请先走到家具旁边');return o;}
export function campAction(s,action,p={}){
 E.assert(!s.ended,'角色已无法行动');
 if(action==='campEnter'){E.assert(!s.player.local&&!s.player.camp,'请先返回区域地图');ensureCamp(s);s.player.camp={...CAMP_EXIT};E.tick(s,1);revealCamp(s);E.log(s,'进入庇护所营地。');return;}
 E.assert(s.player.camp&&campAt(s)?.layout,'请先进入营地');const camp=campAt(s),l=camp.layout;
 if(action==='campExit'){E.assert(s.player.camp.x===7&&s.player.camp.y===14,'请先走到南侧入口');E.tick(s,1);delete s.player.camp;E.log(s,'离开营地，返回区域地图。');return;}
 if(action==='campMove'){const to={x:Number(p.x),y:Number(p.y)};E.assert(campVisible(s,to.x,to.y),'只能向当前可见位置移动');const path=campPath(l,s.player.camp,to);E.assert(path?.length,'这里不可到达');for(const next of path){if(!campVisible(s,next.x,next.y))break;s.player.camp=next;E.tick(s,campStepMinutes(s));revealCamp(s);if(s.ended)break;}return;}
 if(action==='campPlace'||action==='campRelocate'){
  ...
  E.assert(Math.abs(o.x-s.player.camp.x)+Math.abs(o.y-s.player.camp.y)<=3,'请走近施工位置（三格以内）');E.assert(campVisible(s,o.x,o.y),'施工位置不可见');const error=placementError(l,o,s.player.camp,old?.id);E.assert(!error,error);
