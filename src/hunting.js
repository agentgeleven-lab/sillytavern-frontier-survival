...
export function huntingAction(s,action,id){
 ...
 if(action==='cookMeat'||action==='processHide'){
  ...
  E.assert(cook?cookingReady(s):(s.bag.tool??0)>0,cook?'请靠近燃烧中的营火，燃料需至少 10 分钟':'加工皮料需要工具');
 ...
 const {m,a,distance}=selectedAnimal(s,id),name=SPECIES[a.species].name;
 E.assert(distance<=(action==='scareAnimal'?3:1),action==='scareAnimal'?'请靠近到三格内':'请先走到目标所在格或相邻格');
 ...
 if(action==='attackAnimal'||action==='scareAnimal'){
  E.assert(a.hp>0,'这只动物已经死亡');const attack=action==='attackAnimal',damage=(s.bag.tool??0)>0?3:1;
  const roll=randomAt(`${s.seed}:${a.id}:${action}`,s.time,s.revision),success=attack||roll<SCARE_CHANCE[a.species];
  s.stats.stamina-=rule.stamina;
  if(attack){a.hp=Math.max(0,a.hp-damage);if(!a.hp)killAnimal(m,a,s.time);}
  if(a.hp>0&&success){a.fear={x:s.player.local.x,y:s.player.local.y,until:s.time+5};a.state='flee';}
  const counter=a.hp>0&&distance<=1&&(attack?roll<.5:!success)?RETALIATION[a.species]:0;
  E.log(s,attack?`近战攻击${name}，造成 ${damage} 点伤害${a.hp?'。':'，动物死亡。'}`:`驱赶${name}${success?'成功，动物尝试逃离。':'失败，未能持续驱离动物。'}`);
  E.tick(s,rule.minutes,0);
  if(counter){s.stats.health=Math.max(0,s.stats.health-counter);s.ended=s.stats.health<=0;E.log(s,`${name}在接触中反击，损失 ${counter} 点健康${s.ended?'，角色已无法行动':''}。`);}return;
 }
 ...
 E.assert(['butcherAnimal','skinAnimal'].includes(action),'未知尸体操作');E.assert(a.examinedAt!==undefined,'请先检查尸体');E.assert((s.bag.tool??0)>0,'分割和剥皮需要工具');
