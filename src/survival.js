...
export function survivalAction(s,action,item){
 ...
 if(action==='buildFacility'){
  E.assert(camp&&Object.hasOwn(FACILITIES,item),'请在区域地图的营地建设');E.assert(!camp.layout,'请在营地布置模式中放置设施');const r=FACILITIES[item];E.assert(!camp.facilities?.[item],'设施已经建成');E.assert(!r.requires||camp.facilities?.[r.requires],'请先建设灶台');
