...
export function benefitsPanel(s){const rows=activeBenefits(s);return `<section class="fs-survival-panel fs-benefits"><h3>身体状况 · 正面状态</h3>${rows.map(r=>`<p><b>${r.name}</b> · 剩余 ${r.remaining} 分钟<br><small>${r.effect}</small></p>`).join('')||'<p>当前没有正面状态。</p>'}<p class="fs-muted">饱腹满足：饱食低于 80 时吃熟肉、熏肉或新菜肴，达到 80，持续 4 小时。充分休息：在有遮蔽处完整睡够 6 小时，醒来体力至少 80、饱食水分高于 20，且没有出血或感染，持续 6 小时。同名状态只刷新，不叠加；继续吃撑不会延长满足。状态不治疗伤势，仅随游戏时间变化。</p></section>`;}
